...
struct RestaurantListView: View {
    ...
    var body: some View {
        NavigationStack {   // 建立導覽視圖
            List {
                ForEach(restaurants.indices, id: \.self) { index in
                    // 在每個restaurant物件都建立一個導覽連結
            ...
            .navigationBarTitleDisplayMode(.automatic)  // 一開始會是.large，隨著滾動會變成.inline
        }
        .tint(.white)   // 返回按鈕的顏色
...
struct BasicTextImageRow: View {
    ...
    var body: some View {
        ...
        // .sheet監看showOptions的變化，當其為true時，App會帶出動態視圖控制器
